...
sigma_dict = {"sigma1" : [m.sigma1 - m.sigma1*0.2, m.sigma1 + m.sigma1*0.2, "Z1", "Agriculture"],
            "sigma2" : [m.sigma2 - m.sigma2*0.2, m.sigma2 + m.sigma2*0.2, "Z2", "Service"],
            "sigma3" : [m.sigma3 - m.sigma3*0.2, m.sigma3 + m.sigma3*0.2, "Z3", "Industry"],
            "sigmar" : [m.sigmar - m.sigmar*0.2, m.sigmar + m.sigmar*0.2, "Zr", "Refineries"]}

sigma = "sigmar"
reference = m.sigmar


sigma_list = np.linspace(sigma_dict[sigma][0], sigma_dict[sigma][1], 50)
...
for p in sigma_list:
    ...
    print(res.message, p, variables[sigma_dict[sigma][2]])
    
    if res.message == "Optimization terminated successfully":

        res_list_Z.append(variables[sigma_dict[sigma][2]])
...
ax.set_xlabel("Armington Substitution Elasticity Values", fontweight = "bold")
...
ax.legend()
plt.tight_layout()
fig.savefig(f"SensitivityResults/Fig8_{sigma_dict[sigma][3]}_Armington_Sensitivity_Analysis", dpi=150)
