...
omega_dict = {"omega1" : [m.omega1 - m.omega1*0.2, m.omega1 + m.omega1*0.2, "Z1", "Agriculture"],
            "omega2" : [m.omega2 - m.omega2*0.2, m.omega2 + m.omega2*0.2, "Z2", "Service"],
            "omega3" : [m.omega3 - m.omega3*0.2, m.omega3 + m.omega3*0.2, "Z3", "Industry"],
            "omegar" : [m.omegar - m.omegar*0.2, m.omegar + m.omegar*0.2, "Zr", "Refineries"],
            "omegab" : [m.omegab - m.omegab*0.2, m.omegab + m.omegab*0.2, "Zr", "BOTAS"]}
...
omega_list = np.linspace(omega_dict[omega][0], omega_dict[omega][1], 50)
...
for p in omega_list:
    ...
    var_names = m.init_values_str
    init_values = np.array(m.init_values)
    var_values = res.x
    pct_change = (var_values - init_values) / init_values * 100
    variables = dict(zip(var_names, pct_change))

    print(res.message, p, variables[omega_dict[omega][2]])
    
    if res.message == "Optimization terminated successfully":

        res_list_Z.append(variables[omega_dict[omega][2]])
        valid_rho_list.append(p)
...
negative_region_x_axis = valid_rho_list[0]
reversed_region_x_axis = valid_rho_list[-1]


print("reference: ", reference)
print("reversed_region_x_axis: ", reversed_region_x_axis)
...
for i in res_list_Z[1:]:
   
    if sign(i) != s:
        negative_region_x_axis = valid_rho_list[res_list_Z.index(i)]
        break

for j in range(len(res_list_Z[:-1])):

    if (res_list_Z[j] > 0 and res_list_Z[j+1] > 0) and res_list_Z[j] > res_list_Z[j+1] and np.all(np.diff(res_list_Z) > 0):
        reversed_region_x_axis = valid_rho_list[j]
        break  
...
if negative_region_x_axis > reference:
    reversed_region_x_axis = valid_rho_list[0]
...
ax.plot(valid_rho_list, res_list_Z, color = "red", linewidth = 2, label = "Production Change")


ax.axvline(x = negative_region_x_axis, color = "blue", linestyle = "dashed", linewidth = 2)
ax.axvline(x = reference, color = "black", linestyle = "dashed",  linewidth = 2)
ax.axvline(x = reversed_region_x_axis, color = "blue", linestyle = "dashed", linewidth = 2)
ax.axvspan(negative_region_x_axis, reversed_region_x_axis, color="lightgreen", alpha=0.5)

if negative_region_x_axis != valid_rho_list[0]:
    ax.axhline(y = 0, linestyle = "dashed", color = "black")

ax.set_xlabel("CES Substitution Elasticity Values", fontweight = "bold")
ax.set_ylabel("Production (% Change)", fontweight = "bold")




if negative_region_x_axis < reversed_region_x_axis:
    l_bond = negative_region_x_axis
    u_bond = reversed_region_x_axis
else:
    l_bond = reversed_region_x_axis
    u_bond = negative_region_x_axis


lower_bound_percentage = (l_bond - reference) / reference *100
upper_bound_percentage = (u_bond - reference) / reference *100
print(upper_bound_percentage, u_bond)
...
ax.text(
    l_bond,    
    res_list_Z[res_list_Z.index(min(res_list_Z))],          
    f"lower bound: %{round(lower_bound_percentage, 3)}",             
    color="blue",
    fontsize=8,
    ha="center",                
    va="bottom",               
...
ax.text(
    u_bond,    
    res_list_Z[res_list_Z.index(min(res_list_Z))],          
    f"upper bound: %{round(upper_bound_percentage,3)}",             
    color="blue",
    fontsize=8,
    ha="center",                
    va="bottom",               
...
ax.legend()
plt.tight_layout()
fig.savefig(f"SensitivityResults/Fig8_{omega_dict[omega][3]}_CES_Sensitivity_Analysis", dpi=150)
