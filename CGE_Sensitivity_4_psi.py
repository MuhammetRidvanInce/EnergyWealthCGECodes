...
psi_dict = {"psi1" : [m.psi1 - m.psi1*0.2, m.psi1 + m.psi1*0.2, "Z1", "Agriculture"],
            "psi2" : [m.psi2 - m.psi2*0.2, m.psi2 + m.psi2*0.2, "Z2", "Service"],
            "psi3" : [m.psi3 - m.psi3*0.2, m.psi3 + m.psi3*0.2, "Z3", "Industry"],
            "psir" : [m.psir - m.psir*0.2, m.psir + m.psir*0.2, "Zr", "Refineries"],
            "psib" : [m.psib - m.psib*0.2, m.psib + m.psib*0.2, "Zb", "BOTAS"]}

psi = "psib"
reference = m.psib


psi_list = np.linspace(psi_dict[psi][0], psi_dict[psi][1], 50)
...
for p in psi_list:
    ...
    print(res.message, p, variables[psi_dict[psi][2]])
    
    if res.message == "Optimization terminated successfully":

        res_list_Z.append(variables[psi_dict[psi][2]])
...
ax.set_xlabel("CET Transformation Elasticity Values", fontweight = "bold")
...
ax.legend()
plt.tight_layout()
fig.savefig(f"SensitivityResults/Fig8_{psi_dict[psi][3]}_CET_Sensitivity_Analysis", dpi=150)
